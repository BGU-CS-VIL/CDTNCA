...
class CalcTransCPU : public OpKernel {
    public:
        ...
        void Compute(OpKernelContext* context) override {
            ...
            const Tensor& circularity_in = context->input(4);
...
//            const int circularity = (circularity_in.flat<int>()).data();
            const int circularity = 0;
            // Call function
            cpab_forward_op(newpoints, points, trels, nstepsolver, nc,
                            ndim, nP, batch_size, broadcast, circularity);
            return;
        } // end compute method
};
...
class CalcTransGPU : public OpKernel {
    public:
        ...
        void Compute(OpKernelContext* context) override {
            ...
            const auto nstepsolver = (nstepsolver_in.flat<int>()).data();
            const auto nc = (nc_in.flat<int>()).data();
//            const int circularity1 = (circularity_in.flat<int>()).data();
            const int circularity = 0;

//            const auto circularity = circularity_in.item<int>();
//            if(*circularity==1)
//            std::cout<<"Circ value "<<circularity<<std::endl;

//            std::cout<<"Circ dims "<<circularity_in<<std::endl;

//            std::cout<<"Circ value "<<circularity<<std::endl;
            
            // Call solver
            cpab_cuda_forward(eigen_device, points, trels, nstepsolver, nc, 
                            broadcast, ndim, nP, batch_size, newpoints, circularity);
            return;
        } // end compute method
}; // end CalcTransGPU
...
class CalcGradCPU : public OpKernel {
    public:
        ...
        void Compute(OpKernelContext* context) override {
            ...
            const Tensor& circularity_in = context->input(5);
...
//            const int circularity = (circularity_in.flat<int>()).data();
            ...
            cpab_backward_op(grad, points, As, Bs, nstepsolver, nc,
                            n_theta, d, ndim, nP, nC, broadcast, circularity);
            return;
        } // end compute method
}; // end CalcGradCPU
...
//            const int circularity = (circularity_in.flat<int>()).data();
            ...
            cpab_cuda_backward(eigen_device, points, As, Bs, nstepsolver, 
                                nc, broadcast, ndim, nP, n_theta, d, nC, grad, circularity);
            return;
        } // end compute method
}; // end CalcGradGPU
