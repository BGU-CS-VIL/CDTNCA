...
class _CPABFunction_AnalyticGrad(torch.autograd.Function):
    ...
    def forward(ctx, points, theta, params):
        ...
        circ = torch.tensor(params.circularity, dtype=torch.int32, device=device)
        ...
        ctx.save_for_backward(points, theta, As, Bs, nstepsolver, nc, circ)
        # Output result
        return newpoints
    ...
    def backward(ctx, grad): # grad [n_theta, ndim, n]
        # Grap input
        points, theta, As, Bs, nstepsolver, nc, circ = ctx.saved_tensors
